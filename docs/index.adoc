= Static websites with Gatsby and Enonic XP
:toc: right
:imagesdir: media
:src: ../src
...
[NOTE]
====
This guide assues you are already familiar with Gatsby.js, Enonic XP and the headless API in particular. If not, we recommed that looking into these tutorials before commencing:
...
== Introduction
Gatsby site are created from various data sources. Throughout this guide we will use Enonic XP's headless API, also known as https://developer.enonic.com/docs/headless-cms/stable[project Guillotine].

The headless CMS API is based on graphQL, which is a natural companion for Gatsby as well. Throughout this guide we will be using a dataset called the https://developer.enonic.com/templates/headless-cms/[Headless Move Database].
...
. Launch Enonic XP and deploy the headless starter in a clean sandbox
+
...
+
NOTE: This time we're fetching some more data for persons, such as person images, as we will them later. 
+
...
----
+
NOTE: We don't need a new template for persons, as we can reuse the template used for listing movies. 
+
...
+
key:: Specify the field in the query whose value will be used as key for the details page url. In our case, we will be using `name`,
+
