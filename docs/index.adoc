...
{
  guillotine {
    query(contentTypes: "com.example.myproject:movie", query: "valid='true'", first: 100) {
      name: _name
      id: _id
      displayName
...
The query retrieves the fields from movie content type which we will show on the site pages, like movie name, abstract, release date,
images etc.
...
{
  guillotine {
    query(contentTypes: "com.example.myproject:person", query: "valid='true'", first: 100) {
      name: _name
      id: _id
      displayName
...
<3> (optional) Name of the application (the one you specify when creating a new project with Enonic CLI). This name will be
